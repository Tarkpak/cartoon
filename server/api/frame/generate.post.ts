...
function isCharacterNameMatched(sourceName: string, targetName: string): boolean {
  const source = normalizeCharacterName(sourceName)
  const target = normalizeCharacterName(targetName)
  if (!source || !target) return false

  if (source === target) return true

  if (source.length >= 2 && target.length >= 2 && (source.includes(target) || target.includes(source))) {
    return true
  }
...
function findCharacterAsset(
  ...
  if (!characterAssets) return undefined

  const direct = characterAssets[characterName]
  if (direct) return direct

  for (const [assetName, assetValue] of Object.entries(characterAssets)) {
    if (isCharacterNameMatched(characterName, assetName)) {
      return assetValue
    }
  }
...
function hasCharacterAsset(characterName: string, characterAssets?: Record<string, string>): boolean {
  return !!findCharacterAsset(characterName, characterAssets)
}
...
async function generateFirstFrame(
  ...
  if (characterAssets) {
    for (const char of scene.characters) {
      const charImage = findCharacterAsset(char.name, characterAssets)
      if (charImage) {
        if (!referenceImages.includes(charImage)) {
          referenceImages.push(charImage)
        }
        console.log(`[FrameGen] 添加角色立绘参考图: ${char.name}`)
      } else {
        console.warn(`[FrameGen] 未找到首帧角色参考图: ${char.name}`)
      }
    }
  }
...
function buildCharacterConsistencyPrompt(
  ...
  if (!characterAnchors || characterAnchors.length === 0) {
    ...
    const charNames = sceneCharacters.filter(c => hasCharacterAsset(c.name, characterAssets)).map(c => c.name)
...
⚠️ 禁止改变角色的任何外观特征，包括发型、发色、服装颜色和款式。`
  ...
  const anchorsInScene = characterAnchors.filter(anchor =>
    sceneCharacters.some(c => isCharacterNameMatched(c.name, anchor.name))
  )
...
function buildContinuityFirstFramePrompt(
  ...
  const charactersDesc = characters.map((c) => {
    const parts = [c.name]
    const hasAsset = hasCharacterAsset(c.name, characterAssets)
  ...
  if (characterAssets) {
    for (const char of scene.characters) {
      ...
      if (charImage) {
        ...
        console.warn(`[FrameGen] 未找到尾帧角色参考图: ${char.name}`)
      }
    }
  }
