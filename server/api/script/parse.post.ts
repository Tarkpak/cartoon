...
const VALID_ENVIRONMENT_CAPTURE_MODE = new Set(['single', 'four_view'])
...
const ENVIRONMENT_CAPTURE_MODE_PROMPT_RULES = [
  '【环境视角打标（必须执行）】',
...
function appendEnvironmentCaptureModeRule(prompt: string): string {
  if (!prompt?.trim()) return prompt
  if (prompt.includes('environmentCaptureMode')) return prompt
...
function inferEnvironmentCaptureModeFromSceneText(options: {
  description?: string
  cameraNote?: string
}): 'single' | 'four_view' {
  ...
  if (countShotKeywordKinds(description) >= 2) {
    return 'four_view'
  }
...
function normalizeSceneEnvironmentCaptureMode(
  rawMode: unknown,
  fallback: {
    description?: string
    cameraNote?: string
  ...
  if (typeof rawMode === 'string') {
    ...
    if (VALID_ENVIRONMENT_CAPTURE_MODE.has(value)) {
      return value as 'single' | 'four_view'
    }
  }

  return inferEnvironmentCaptureModeFromSceneText(fallback)
}
...
function normalizeParsedScriptOutput(
  ...
  const normalizedScenes = rawScenes.map((scene, index) => {
    ...
    return {
      ...
      environmentCaptureMode: normalizeSceneEnvironmentCaptureMode(
        sceneObj.environmentCaptureMode ?? sceneObj.environment_capture_mode,
        {
          description: typeof sceneObj.description === 'string' ? sceneObj.description : '',
          cameraNote: typeof sceneObj.cameraNote === 'string'
            ? sceneObj.cameraNote
            : (typeof sceneObj.camera_note === 'string' ? sceneObj.camera_note : '')
        }
      ),
      dramatic,
...
export async function executeScriptParse(
  ...
  const runScriptParsePass = async (options: {
    ...
    const promptWithEpisodeDrama = episodeContextBrief
      ...
      : prompt
    const promptWithEnvironmentCaptureMode = appendEnvironmentCaptureModeRule(promptWithEpisodeDrama)

    const result = await generateJSONForWorkflow<ParsedScript>('script_parsing', {
      prompt: promptWithEnvironmentCaptureMode,
      temperature: 0.3,
      maxRetries: 2
    })
