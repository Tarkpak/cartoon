...
function estimateRecommendedMinScenes(textLength: number): number {
  // 按文本长度线性估算场景下限，避免把长文本压缩成少量场景。
...
export default defineEventHandler(async (event) => {
  ...
  const { text, workflowType, style, maxScenes } = parseResult.data
  const normalizedWorkflow = normalizeProjectWorkflowType(workflowType)

  try {
    const textLength = text.trim().length
    let recommendedMinScenes = estimateRecommendedMinScenes(textLength)
