...
Manju 是一个 AI 驱动的影视创作平台，实现从文本到视频的全自动化生成流程。支持首尾帧控制、角色一致性、智能配音，目标是降低影视内容制作门槛和成本。
...
│  │  - settings.vue   │ - Timeline       │ - useAssetWorkbenchSceneGeneration() │  │
│  └───────────────────────────────────────────────────────────┘  │
│                              │                                   │
│                              ▼                                   │
│  ┌───────────────────────────────────────────────────────────┐  │
...
| `useScript()` | 剧本解析 |
| `useCharacter()` | 角色生成 |
| `useAssetWorkbenchSceneGeneration()` | 场景参考图与视频生成 |
| `useWorkbench()` | 工作台集成 |
